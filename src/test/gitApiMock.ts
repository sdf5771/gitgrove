...
export function installGitApiMock() {
  ...
  const gitAPI = {
    ...
    unstage: vi.fn(async () => {}),
    commit: vi.fn(async () => {}),
    pull: vi.fn(async () => ({ success: true, op: 'pull' as const, summary: '' })),
    push: vi.fn(async () => ({ success: true, op: 'push' as const, summary: '' })),
    fetch: vi.fn(async () => ({ success: true, op: 'fetch' as const, summary: '' })),
    onRemoteProgress: vi.fn(() => () => {}),
