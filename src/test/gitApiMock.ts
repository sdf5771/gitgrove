...
export function installGitApiMock() {
  ...
  const gitAPI = {
    ...
    listTags: vi.fn(async () => [] as GitTagEntry[]),
    deleteTag: vi.fn(async () => {}),
    pushTag: vi.fn(async () => {}),
    stashApply: vi.fn(async () => {}),
    stashDrop: vi.fn(async () => {}),
    stashList: vi.fn(async () => []),
    stashFiles: vi.fn(async () => []),
    stashPreview: vi.fn(async () => ({ tracked: [], untracked: [] })),
    stashPush: vi.fn(async () => true),
    stashBranch: vi.fn(async () => {}),
    stashFileDiff: vi.fn(async () => ''),
    ...
    getMergeState: vi.fn(async (): Promise<MergeState> => ({ op: null, conflictedCount: 0 })),
    continueMerge: vi.fn(async (): Promise<ContinueResult> => ({ ok: true })),
  }
  ...
  const appAPI = {
    ...
    checkUpdates: vi.fn().mockResolvedValue({ updateAvailable: false, current: '1.19.9' }) as Mock<() => Promise<{ updateAvailable: boolean; version?: string; dmgUrl?: string; current: string }>>,
