...
// 재설계 후: GitLab은 '서비스 연결' 탭의 인라인 연결 흐름. initialTab="gitlab"로 바로 연다.
function openGitlabFlow() {
  render(<SettingsPanel onClose={vi.fn()} repoPath={null} initialTab="gitlab" />)
}

describe('SettingsPanel — GitLab 연결 흐름 (서비스 연결 탭)', () => {
  ...
  it('연결 해제 시 host-키 토큰을 제거하고 미연결 상태로 돌아간다', async () => {
    getCurrentUserMock.mockResolvedValue(glUser())
    openGitlabFlow()
    ...
    expect(screen.queryByText('@seo-kim')).toBeNull()
    // 미연결 GitLab 카드로 복귀
    expect(screen.getAllByText("미연결").length).toBeGreaterThanOrEqual(1)
  })
  ...
  it('마운트 시 기존 연결 host가 있으면 연결됨 카드로 복원한다', async () => {
