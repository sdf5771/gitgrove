...
export function InteractiveRebaseModal({ onClose, onSuccess, repoPath, commits, currentBranch }: Props) {
  ...
  return (
    <ModalShell title="대화형 리베이스" width={560} onClose={onClose} icon={icon}>
      {isDone ? <SuccessState msg="리베이스 완료" sub={`${pickCount}개 적용 · ${dropCount}개 제거`} /> : (
        <>
          <div style={{ padding: '10px 16px 6px', fontSize: 11, color: 'var(--c-text-muted)', borderBottom: '1px solid var(--c-divider)' }}>
            <strong style={{ color: 'var(--c-gold-300)', fontFamily: 'var(--font-mono)' }}>{branch}</strong>에서 최근 {items.length}개 커밋을 리베이스해요 · 배지를 눌러 동작 변경 · ↑↓로 순서 변경
          </div>
          <div className="modal-body" style={{ padding: '10px 12px' }}>
            ...
            <div style={{ marginTop: 10, padding: '7px 10px', background: 'var(--c-bg-inset)', border: '1px solid var(--c-divider)', borderRadius: 'var(--r2)', fontSize: 11, color: 'var(--c-text-muted)', display: 'flex', gap: 10 }}>
              <span>기준 · <strong style={{ color: 'var(--c-text)', fontFamily: 'var(--font-mono)' }}>{baseHash}</strong></span>
          ...
          <div className="modal-footer">
            ...
            <button className={`mbtn-ok${doing ? ' doing' : ''}`} onClick={execute} disabled={doing}>
              {doing ? <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ display: 'inline-block', animation: 'spin 600ms linear infinite' }}>⟳</span>진행 중…</span> : '리베이스 시작 →'}
            </button>
          </div>
        </>
      )}
